    ...
    : IEverWallet
{
    ...
    {
        Value = JsonSerializer.Deserialize<AbiContract>(WalletContractAbiJson,
    ...
    {
        var address = await memoryCache.GetOrCreateAsync<string>($"AddressByStateInitBoc_{stateInitBoc}",
            async entity =>
            {
