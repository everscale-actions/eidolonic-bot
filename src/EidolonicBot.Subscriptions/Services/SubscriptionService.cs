...
internal class SubscriptionService : IHostedService, ISubscriptionService, IAsyncDisposable {
  ...
  public async Task StopAsync(CancellationToken cancellationToken) {
    ...
    await _everClient.Net.Unsubscribe(
      new ResultOfSubscribeCollection { Handle = _handler.Value },
      cancellationToken);
  }
  ...
  public async Task Reload(CancellationToken cancellationToken) {
    ...
    if (oldHandler is not null) {
      await Policy
        ...
        .WaitAndRetryAsync(10, retryAttempt => TimeSpan.FromSeconds(Math.Pow(2, retryAttempt)))
        .ExecuteAsync(
          ct => _everClient.Net.Unsubscribe(
  ...
  private async Task StartSubscription(CancellationToken cancellationToken) {
    ...
    _handler = null;
    var resultOfSubscribeCollection = await _everClient.Net.Subscribe(
      new ParamsOfSubscribe {
